    ...
    try {
      ...
      const res = await fetch(
        `${base.replace(/\/?$/, "/")}collections/${selectedCollection.uuid}/documents/search`,
        {
          ...
          body: JSON.stringify({ query: ragQuery, limit: 10 }),
        },
      );
      if (!res.ok) throw new Error(await res.text());
      const data = await res.json();
      const aggregated = Array.isArray(data)
        ? data.map((d: any) => d.page_content).join("\n\n")
        : JSON.stringify(data);
      setRagResults(aggregated || "No results");
    } catch (e) {
      setRagResults(String(e));
    } finally {
        ...
        <div className="mb-6">
          ...
          {selectedCollection && (
            <div className="mb-4 flex items-center gap-2">
              <Input
                placeholder="Search collection with RAG Anything (hybrid)"
                value={ragQuery}
                onChange={(e) => setRagQuery(e.target.value)}
              />
              <Button size="sm" onClick={runRagSearch} disabled={!ragQuery.trim() || ragSearching}>
                {ragSearching ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : null}
                Search
              </Button>
            </div>
          )}
          {ragResults && (
            <div className="mb-4 max-h-64 overflow-auto rounded border p-3">
              <pre className="whitespace-pre-wrap break-words text-xs leading-snug">{ragResults}</pre>
            </div>
          )}
