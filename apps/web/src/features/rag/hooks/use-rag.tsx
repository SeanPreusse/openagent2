...
export function useRag(): UseRagReturn {
  ...
  const handleFileUpload = useCallback(
    async (files: FileList | null, collectionId: string) => {
      ...
      const clientSideMetas = Array.from(files).map((file) => ({
        name: file.name,
      ...
      // Refresh from server to get canonical IDs and metadata
      const refreshed = await listDocuments(collectionId, { limit: 100 }, session.accessToken);
  ...
  const handleTextUpload = useCallback(
    async (textInput: string, collectionId: string) => {
      ...
      await uploadDocuments(collectionId, [textFile], session.accessToken, [metadata]);
