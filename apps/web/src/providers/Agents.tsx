    ...
    async (deployment) => {
      ...
      const assistantsPromise: Promise<Agent[]>[] =
        assistantsGroupedByGraphs.map(async (group) => {
          ...
          // For user-created agents, determine supported configs individually
          // For default agents, use the schema from the template
          return await Promise.all(group.map(async (assistant) => {
            let supportedConfigs: string[] = [];
            
            if (schema) {
              // For user-created agents, check their actual config to determine support
              const agentToAnalyze = isUserCreatedDefaultAssistant(assistant) ? defaultAssistant : assistant;
              const { toolConfig, ragConfig, agentsConfig } =
                extractConfigurationsFromAgent({
                  agent: agentToAnalyze,
                  schema,
                });
...
export const AgentsProvider: React.FC<{ children: ReactNode }> = ({
  ...
  async function refreshAgents() {
    ...
    try {
      ...
      const filteredAgents = newAgents.filter((a) => {
        const isSystem = isSystemCreatedDefaultAssistant(a);
        if (isSystem) {
          console.warn("Filtering out system-created agent:", {
            id: a.assistant_id,
            name: a.name,
            metadata: a.metadata,
            created_by: a.metadata?.created_by,
            reason: "Agent marked as system-created - should be preserved with metadata fix"
          });
        }
        return !isSystem;
      });
      console.log("Refresh agents result:", {
        totalFetched: newAgents.length,
        afterFiltering: filteredAgents.length,
        filtered: newAgents.length - filteredAgents.length,
        filteredAgentNames: newAgents.filter(a => isSystemCreatedDefaultAssistant(a)).map(a => a.name)
      });
